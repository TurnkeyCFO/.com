(function () {
  ...
  track('lp_view', { lp: 'treeservice', lp_version: 'v1' });
  ...
  window.addEventListener('message', function (e) {
    ...
    if (evt.indexOf('calendly') === 0 && evt === 'calendly.event_scheduled') {
      track('calendly_booking_complete', { lp: 'treeservice', conversion: 1 });
      if (typeof gtag === 'function') {
        gtag('event', 'conversion', { send_to: 'AW-XXXXXXXXX/XXXXXXXXXXX' });
      }
    }
    if (evt === 'estimate_submitted' || evt === 'instant_estimate_submitted') {
      track('estimate_submit_complete', { lp: 'treeservice', conversion: 1 });
      if (typeof gtag === 'function') {
  ...
  function depth() {
    ...
    [25, 50, 75, 90].forEach(function (t) {
      if (pct >= t && !fired[t]) {
        fired[t] = true;
        track('scroll_depth', { depth: t, lp: 'treeservice' });
      }
    });
  }
